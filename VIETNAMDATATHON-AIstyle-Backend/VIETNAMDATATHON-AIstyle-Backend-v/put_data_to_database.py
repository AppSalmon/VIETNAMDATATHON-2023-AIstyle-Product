...
brand_nike = Brand(
    name="Nike",
    description="Nike, Inc. is a globally renowned American multinational corporation that is engaged in the design, development, manufacturing, and marketing of footwear, apparel, equipment, accessories, and services. The company is one of the world's largest and most recognizable athletic and sportswear brands, and it has established itself as a symbol of innovation, performance, and style.",
)
brand_adidas = Brand(
    name="Adidas",
    description="Adidas is a multinational corporation that is widely recognized for its presence in the sportswear and athletic footwear industry. The company was founded on August 18, 1949, by Adolf Dassler in Herzogenaurach, Germany. The name Adidas is a combination of the founder's nickname, Adi, and the first three letters of his last name.",
)
...
for index, row in df_crawl.iterrows():
    ...
    if row["brand"] != "adidas":
        ...
        link = row["images"][2:-2]
        image = ImageLink(image=link, product_detail_id=productId)
        with app.app_context():
...
for index, row in df_btc.iterrows():
    ...
    for i in range(len(links)):
        if i == 4:
            break
        image = ImageLink(image=link, product_detail_id=productId2)
        with app.app_context():
            db.session.add(image)
            db.session.commit()
